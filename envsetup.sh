...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^proandroid_") ; then
       PROANDROID_BUILD=$(echo -n $1 | sed -e 's/^proandroid_//g')
    else
       PROANDROID_BUILD=
    fi
    export PROANDROID_BUILD
...
{
    target=$1
    PROANDROID_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/proandroid/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the proandroid model name
            lunch proandroid_$target-userdebug
        fi
    fi
    return $?
}
